		...
		if(viewType!=null && "enquirysubmit".equalsIgnoreCase(viewType)) {
			...
			try {
				if(!enquiryDao.insertEnquiryDetails(enquiry)) {
					request.setAttribute("errorMsg", "Failed to raise ticket. Please try again!");
				}else {
					request.setAttribute("errorMsg", "Ticket posted successfully. Please wait for 1-2 business days to recieve response!");
				}
			...
			Enquiry enquiry = new Enquiry();
			if(searchString!=null && !searchString.isEmpty() && !"select".equalsIgnoreCase(searchString)) {
			...
			try {
				if(!enquiryDao.updateEnquiryDetails(enquiry)) {
					request.setAttribute("errorMsg", "Failed to update ticket details. Please try again!");
				}else {
					request.setAttribute("errorMsg", "Ticket details updated successfully!");
				}
			...
			if(searchString!=null && !searchString.isEmpty() && !"select".equalsIgnoreCase(searchString)) {
				enquiry1.setServiceType(searchString.trim());
			}
			List<Enquiry> enquiryList = enquiryDao.getEnquiryDetails(enquiry1);
